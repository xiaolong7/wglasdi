...
def err_indicator(u_sim, params, data=None, err_type=1, idx_start=0):
    """
    ...
    """
    if err_type == 1:
        # err = (np.linalg.norm(data[idx_start:] - u_sim[idx_start:], axis=1) / np.linalg.norm(data[idx_start:], axis=1)*100).max()
        data_max = np.linalg.norm(data, axis=1).max()
        err = (np.linalg.norm(data - u_sim, axis=1) / data_max*100).max()
    elif err_type == 2:
        res = []
